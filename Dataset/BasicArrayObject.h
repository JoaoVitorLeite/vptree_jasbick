    ...
    public:
        ...
        */
        uint32_t getSerializedSize(){
            ...
            return (sizeof(uint32_t) + sizeof(size_t) +  (sizeof(DType) * data.size()));
        }
        ...
        */
        const char *serialize(){
            ...
            if (serialized == NULL){
                ...
                memcpy(serialized + sizeof(uint32_t), &size, sizeof(size_t));
                for (int x = 0; x < size; x++){
                    memcpy(serialized + sizeof(uint32_t) + sizeof(size_t)+(sizeof(DType)*x), &data[x], sizeof(DType));
                }
            }
            return serialized;
        }
        ...
        */
        void unserialize(const char *dataIn, size_t dataSize = 0){
            ...
            if (dataSize != 0) {
                size_vector = (dataSize - sizeof(uint32_t) - sizeof(size_t)) / sizeof(DType);
            } else {
                memcpy(&size_vector, dataIn + sizeof(uint32_t), sizeof(size_t));
            }

            d = new DType[size_vector];

            for (int x = 0; x < size_vector; x++){
                memcpy(&d[x], dataIn + sizeof(uint32_t) + sizeof(size_t) + (sizeof(DType)*x), sizeof(DType));
            }
        ...
        * Use unserialize(const char *dataIn, uint32_t dataSize) instead.
        * @copydoc unserialize(const char *dataIn, size_t dataSize).
        */
        void Unserialize(const char *dataIn, size_t dataSize = 0){
        ...
        */
        void unserializeFromString(std::string dataIn){
            ...
            memcpy(&size_vector, dataIn.c_str() + sizeof(uint32_t), sizeof(size_t));
            d = new DType[size_vector];

            for (int x = 0; x < size_vector; x++){
                memcpy(&d[x], dataIn.c_str() + sizeof(uint32_t) + sizeof(size_t) + (sizeof(DType)*x), sizeof(DType));
            }
